...
from network.backbone.attention import PAM_Module,CAM_Module
...
class DeepLabHead(nn.Module):
    ...
    def forward(self, feature):
        return self.classifier( feature['out'] ),0
...
class DoubleAttentionHead(nn.Module):
    ...
    def forward(self, feature):
        ...
        return self.classifier(output),output1
...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        # self.at = PAM_Module(256)
        # self.conv_at = nn.Conv2d(256, 256, 3, padding=1, bias=False)
    ...
    def forward(self, feature):
        output = self.project(feature['out'])
        # output_at = self.at(output)
        # output = self.conv_at(output_at)
